...
#import "CZYFullVideoViewController.h"

@interface CZYAVPlayerView ()
...
/** 全屏播放控制器 */
@property(nonatomic,strong)CZYFullVideoViewController *fullVc;
...
/**
 全屏之前的容器视图内frame
 */
@property (nonatomic, assign)CGRect contrainerFrame;
...
@implementation CZYAVPlayerView
+ (instancetype)videoPlayView {
    return [[NSBundle mainBundle] loadNibNamed:@"CZYAVPlayerView" owner:self options:nil].firstObject;
}
...
{
    if (isFull) {
        self.contrainerView = self.superview;
        self.contrainerFrame = self.frame;
        ...
        [self.fullVc dismissViewControllerAnimated:NO completion:^{
            ...
            [UIView animateWithDuration:0.25 delay:0.0 options:UIViewAnimationOptionLayoutSubviews animations:^{
                self.frame = self.contrainerFrame;
            } completion:nil];
        }];
    }
}
...
#pragma mark - 懒加载代码
- (CZYFullVideoViewController *)fullVc
{
    if (_fullVc == nil) {
        _fullVc = [[CZYFullVideoViewController alloc] init];
    }
    return _fullVc;
}
